...
create policy if not exists ro_locations on public.locations for select using (true);
create policy if not exists ro_devices   on public.devices   for select using (true);
create policy if not exists ro_events    on public.sensor_events for select using (true);
create policy if not exists ro_scenes    on public.scenes   for select using (true);
create policy if not exists ro_scene_actions on public.scene_actions for select using (true);
create policy if not exists ro_app_settings on public.app_settings for select using (true);

-- SERVICE ROLE full access
create policy if not exists srv_locations on public.locations
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy if not exists srv_devices on public.devices
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy if not exists srv_events on public.sensor_events
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy if not exists srv_scenes on public.scenes
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy if not exists srv_scene_actions on public.scene_actions
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy if not exists srv_app_settings on public.app_settings
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
